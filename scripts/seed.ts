...
async function seed() {
  ...
  if (existingCourses > 0) {
    ...
    const courseDocs = subjects.map((s) => ({
      name: s.name,
      level: s.level,
      category: s.category,
      icon: s.icon,
      ...
      resources: {
        notes: s.resources.notes.map((r) => ({
          title: r.title,
          description: r.description,
          type: r.type,
        })),
        quizzes: s.resources.quizzes.map((r) => ({
          title: r.title,
          description: r.description,
          type: r.type,
        })),
        pastPapers: s.resources.pastPapers.map((r) => ({
          title: r.title,
          description: r.description,
          type: r.type,
        })),
        videos: s.resources.videos.map((r) => ({
          title: r.title,
          description: r.description,
          type: r.type,
        })),
